...
class LineChartButton extends Component {
    constructor(props){
      super(props)
      this.createLineChartButton = this.createLineChartButton.bind(this)
    }
  
    componentDidMount() {
      this.createLineChartButton()
    }
    componentDidUpdate() {
      this.createLineChartButton()
    }
  
    createLineChartButton() {
        // const _self = this;
    ...
    render() {
        return <svg ref={node => this.node = node} transform='translate(0,0)'> 
        </svg>
    }
}
export default function f(flag) {
    return (
        <div>
        ...
        <div class="svg" style={{width:width, height:height, margin:"auto"}}><LineChartButton/>
        {/* </div> */}</div>
        </div>
        // <LineChartButton/>
    );
