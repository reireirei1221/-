...
class City extends Component {
    ...
    //   this.food_code = props.food_code;
    //   console.log("this.food_code",this.food_code);
    //   this.food_code = props.map.food_code;
    }
    ...
    createCityGraph(food_code=this.props.food_code) {
        ...
        function change_style() {
          document.querySelector(".alloffs").style.boxShadow = "0 2px 0 rgb(192, 126, 4)";
          console.log("aa");
        ...
        /* 追加 */
        function onOver(ele) {
          var hover_id = ele.getAttribute("id");
          //ボタンid, 都市コードの辞書
          var hover_code = cities_code_dic[hover_id];
          var path_emp;
          selected_line_plot(hover_code);
        }

        function selected_line_plot(hover_code) {
          //console.log(display_flags);
          if (hover_code != "allon" && hover_code != "allof") {
            if (typeof dataset[food_code][hover_code] == "undefined") continue;
            var selected_dataset = dataset[food_code][hover_code];
            if (display_flags[city_code] == false) continue;
            path_emp = svg.append("path")
              .datum(selected_dataset)
              .attr("fill", "none")
              .attr("stroke", cities_color_dic[hover_code])
              .attr("stroke-width", 2.0)
              .attr("d", d3.line()
                  .x(function(d) { return xScale(set_time(new Date(d[0]))); }) //(new Date(d3.utcDay(new Date(tmp(d)))))
                  .y(function(d) { return yScale(d[1]); }));
              path_emp.attr("id", hover_code + "_line");
          }
          else if (hover_code == "allon") {
  ...
  //変更(東京)
    return (
        <div>
            ...
            <a href="#" id="tokyo" onMouseOut="omOut(this);" onMouseOver="omOver(this);">東京</a> 
